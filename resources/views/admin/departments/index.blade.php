...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="surface-card p-5">
            ...
            <p class="text-3xl font-semibold text-[#001722]">{{ $departments->total() }}</p>
        </div>
        <div class="surface-card p-5">
            <p class="text-sm text-[#084A48]">Active Departments</p>
            <p class="text-3xl font-semibold text-[#084A48]">{{ $departments->where('is_active', true)->count() }}</p>
        </div>
        <div class="surface-card p-5">
            <p class="text-sm text-[#FE580B]">Inactive Departments</p>
            <p class="text-3xl font-semibold text-[#7f2f00]">{{ $departments->where('is_active', false)->count() }}</p>
        </div>
    </div>
    ...
    <div class="surface-card p-5">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('admin.departments.create') }}" class="btn-primary px-4 py-2 inline-flex items-center gap-2">
                    <i class="fas fa-plus"></i>
                    Add Department
                </a>
                <button id="reorderButton" type="button" onclick="toggleReorderMode()" class="btn-secondary px-4 py-2 inline-flex items-center gap-2">
                    <i class="fas fa-sort"></i>
                    Reorder
                </button>
                <button id="saveOrderBtn" type="button" onclick="saveOrder()" class="hidden btn-primary px-4 py-2 inline-flex items-center gap-2">
            ...
            <form method="GET" class="flex items-center gap-2 w-full md:w-auto">
                ...
                <input id="searchInput" type="text" name="search" value="{{ request('search') }}" placeholder="Search departments..." class="form-input w-full md:w-72 px-4 py-2">
                <button type="submit" class="btn-primary w-12 h-12 inline-flex items-center justify-center">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($departments as $department)
                        <tr class="hover:bg-[#001722]/10 transition duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($department->is_active)
                                    <span class="inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full badge-teal">Active</span>
                                @else
...
<div id="deleteModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 z-50 hidden">
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="surface-card max-w-md w-full mx-4 overflow-hidden">
