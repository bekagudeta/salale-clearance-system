    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST" id="editUserForm" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach($roles as $role)
                            @if($role->name !== 'super_admin')
                                ...
                                @endphp
                                <label class="cursor-pointer rounded-2xl border p-4 transition hover:border-blue-500 {{ $selected ? 'border-blue-600 bg-blue-50 shadow-sm' : 'border-gray-200 bg-white' }}">
                                    <input type="radio" name="role" value="{{ $role->name }}" class="sr-only role-radio" {{ $selected ? 'checked' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="w-24 h-24 rounded-lg bg-gray-100 border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden" id="photoPreview">
                                        @if($user->student && $user->student->photo)
                                            <img src="{{ $user->student->photo_url }}" class="w-full h-full object-cover">
                                        @else
                ...
                <div id="officerFields" style="display: {{ $user->hasRole('department_officer') ? 'block' : 'none' }};" class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            <label class="inline-flex items-center gap-3 mt-1">
                                <input type="checkbox" name="can_approve" value="1" class="h-4 w-4 text-blue-600 border-gray-300 rounded" {{ old('can_approve', $user->departments->first()?->pivot->can_approve ?? true) ? 'checked' : '' }}>
